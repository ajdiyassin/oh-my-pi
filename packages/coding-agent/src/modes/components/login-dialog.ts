...
export class LoginDialogComponent extends Container {
	...
	showManualInput(prompt: string): Promise<string> {
		const phase = this.#resetPhase();
		phase.addChild(new Spacer(1));
		phase.addChild(new Text(theme.fg("dim", prompt), 1, 0));
		phase.addChild(this.#input);
		phase.addChild(new Text(theme.fg("dim", "(Escape to cancel)"), 1, 0));
	...
	showPrompt(message: string, placeholder?: string, defaultValue?: string): Promise<string> {
		const phase = this.#resetPhase();
		phase.addChild(new Spacer(1));
		phase.addChild(new Text(theme.fg("text", message), 1, 0));
		if (placeholder) {
			phase.addChild(new Text(theme.fg("dim", `e.g., ${placeholder}`), 1, 0));
		}
		phase.addChild(this.#input);
		phase.addChild(new Text(theme.fg("dim", "(Escape to cancel, Enter to submit)"), 1, 0));
		this.#activeControl = "input";
