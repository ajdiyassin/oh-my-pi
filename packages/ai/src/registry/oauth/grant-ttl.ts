/**
 * OAuth grant lifetime constants. Kept in a dependency-free leaf module so the
 * public barrel can re-export them without pulling an OAuth flow (and its
 * `providers/*` imports) into the `registry` -> `oauth` load path. Re-exporting
 * `./anthropic` from the barrel instead creates the cycle
 * `providers/anthropic -> stream -> registry -> oauth -> oauth/anthropic ->
 * providers/anthropic`, whose load order decides whether `claudeCodeVersion` is
 * initialized — under parallel test execution it intermittently throws
