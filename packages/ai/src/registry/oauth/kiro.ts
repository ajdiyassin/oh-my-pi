...
function readCachedRegisteredClient(
	...
	let invalidTokenEndpoint = false;
	try {
		...
		try {
			tokenEndpoint = resolveKiroTokenEndpoint(data.tokenEndpoint, region);
		} catch (error) {
			if (data.tokenEndpoint !== undefined) {
				invalidTokenEndpoint = true;
				throw error;
			}
			return undefined;
		}
		...
		if (!registrationIsUsable(client)) return undefined;
		cacheRegisteredClient(cache, region, client);
		return client;
	} catch (error) {
		if (invalidTokenEndpoint) throw error;
		return undefined;
	}
}
