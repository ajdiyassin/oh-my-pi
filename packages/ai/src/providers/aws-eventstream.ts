 ...
 *
 * Size bounds match aws-c-event-stream / botocore (24 MiB frames, 128 KiB
 * headers) so a hostile stream cannot force unbounded buffering.
 */

import { EventStreamFrameError } from "../error/aws";
...
function utf8Decoder(): TextDecoder {
	try {
		...
		// Environments without fatal mode still decode; invalid sequences are
		// replaced, which is acceptable only as a fallback.
...
function assertFrameLengths(total: number, headersLen: number): void {
	if (!Number.isFinite(total) || total < MIN_MESSAGE_LEN) {
		throw new EventStreamFrameError(`total length ${total} below minimum`);
	}
	if (total > MAX_FRAME_SIZE) {
		throw new EventStreamFrameError(`oversized frame (${total} > ${MAX_FRAME_SIZE})`);
	}
	if (!Number.isFinite(headersLen) || headersLen < 0) {
		throw new EventStreamFrameError(`invalid headers length ${headersLen}`);
	}
	if (headersLen > MAX_HEADERS_SIZE) {
		throw new EventStreamFrameError(`headers length ${headersLen} exceeds limit ${MAX_HEADERS_SIZE}`);
	}
	if (headersLen > total - MIN_MESSAGE_LEN) {
		throw new EventStreamFrameError(`header block length ${headersLen} does not fit in frame ${total}`);
	}
}
...
export function decodeMessage(frame: Uint8Array): EventStreamMessage {
	if (frame.length < MIN_MESSAGE_LEN) throw new EventStreamFrameError("frame too short");
	...
	if (total !== frame.length) throw new EventStreamFrameError(`framed length ${total} != buffer ${frame.length}`);

	const preludeCrc = view.getUint32(8, false);
	const computedPreludeCrc = crc32(frame.subarray(0, PRELUDE_LEN));
	if (computedPreludeCrc !== preludeCrc) throw new EventStreamFrameError("prelude CRC mismatch");

	const msgCrc = view.getUint32(total - MESSAGE_CRC_LEN, false);
	const computedMsgCrc = crc32(frame.subarray(0, total - MESSAGE_CRC_LEN));
	if (computedMsgCrc !== msgCrc) throw new EventStreamFrameError("message CRC mismatch");
...
function parseHeaders(buf: Uint8Array): Record<string, string> {
	...
	while (p < buf.length) {
		...
		switch (type) {
			...
			default:
				throw new EventStreamFrameError(`unknown header value type ${type}`);
		}
	}
	return out;
}
	...
	try {
		while (true) {
			...
			while (buf.length - offset >= 4) {
				...
				if (total < MIN_MESSAGE_LEN) throw new EventStreamFrameError(`total length ${total} below minimum`);
				if (total > MAX_FRAME_SIZE) {
					throw new EventStreamFrameError(`oversized frame (${total} > ${MAX_FRAME_SIZE})`);
				}
